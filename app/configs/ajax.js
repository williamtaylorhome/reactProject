...
// Prevent multiple user login timeout prompts from appearing continuously
let flag = true
...
export const oftenFetchByPost = (api, options) => {
  // When the api parameter is the return value created by create api
   ...
   * ()
   */
  return (...rest) => { // Parameters: (data:Object,sucess?:Function,failure?:Function,config?:Object)
// Parameter analysis
    const data = rest[0] || {}
    ...
    // If you are using the mock service on 30, then cookies will not be sent to the server by default.
...
// failure && failure(response)
      ...
      .catch((e) => {
        if (axios.isCancel(e)) {
          ...
          console.dir(e)
          if (typeof failure === 'function') {
            if (e.code === 'ECONNABORTED') { // Timeout error
              failure({
                data: '',
                msg: '服务器连接超时',
                status: 0,
...
// Create a launcher that launches the api
